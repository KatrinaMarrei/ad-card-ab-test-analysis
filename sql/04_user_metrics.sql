-- Пользовательская аналитическая витрина: одна строка на пользователя.
WITH user_groups AS (
    SELECT DISTINCT
        user_id,
        experiment_group
...
SELECT
    ...
    users.device,
    users.user_segment,
    COALESCE(event_metrics.views, 0) AS views,
    COALESCE(event_metrics.favorites, 0) AS favorites,
    COALESCE(event_metrics.contacts, 0) AS contacts,
    COALESCE(event_metrics.purchases, 0) AS purchases,
    ROUND(COALESCE(payment_metrics.revenue, 0), 2) AS revenue,
    CASE WHEN COALESCE(event_metrics.contacts, 0) > 0 THEN 1 ELSE 0 END AS has_contact,
    CASE WHEN COALESCE(event_metrics.purchases, 0) > 0 THEN 1 ELSE 0 END AS has_purchase,
    CASE WHEN COALESCE(payment_metrics.revenue, 0) > 0 THEN 1 ELSE 0 END AS is_payer
FROM user_groups
INNER JOIN users
